...
void EPaperUC8151::setup() {
  EPaperBase::setup();
  if (this->is_failed())
    return;
  // Software copy of the previously displayed frame (differential base for
  // partial updates; the panel is deep-slept between updates). Only allocated
  // when partial updates are enabled, saving a full framebuffer of RAM
  // otherwise. PSRAM-capable.
...
bool EPaperUC8151::initialise(bool partial) {
...
bool HOT EPaperUC8151::transfer_data() {
...
void EPaperUC8151::power_on() {
  this->command(0x04);  // POWER ON
}

void EPaperUC8151::refresh_screen(bool /*partial*/) {
  this->command(0x12);  // DISPLAY REFRESH
}

void EPaperUC8151::power_off() {
  this->cmd_data(0x50, {0xF7});  // VCOM and data interval (border floating)
  this->command(0x02);           // POWER OFF
}

void EPaperUC8151::deep_sleep() {
